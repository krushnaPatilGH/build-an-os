...
typedef struct 
{
    ...
    GDTEntry* Address;                      // pointer to gdt
...
{
    i686_GDT_Load(&g_GDTDescriptor, i686_GDT_CODE_SEGMENT, i686_GDT_DATA_SEGMENT);  // the cs and ds used are not address of segments but the address of the offest 
}                                                                                   // of the entries in the gdt i.e cs is 1st entry so offset = sizeOf(entry) * 1 = 8
                                                                                    // for ds offset = sizeOf(entry) * 2 = 16 = 0x10h