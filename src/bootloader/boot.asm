...
.search_kernel:
	...
	repe cmpsb								; cmp str di and si i.e. file kernel and root dir entries
	pop di
	je .found_kernel
...
.found_kernel:
	; di should have the address of the kernel
	mov ax, [di + 26]						; offset of first_cluster_low in the dir entry is 26
	mov [kernel_cluster], ax				; load the cluster of kernel
...
.load_kernel_loop:
	...
	;Read next cluster
	mov ax, [kernel_cluster]				; loc of first cluster must be changed in future
	add ax, 31
...
.next_cluster_after:
	...
	mov [kernel_cluster] , ax				; load the value of next cluster into [kernel_cluster]
...
msg_kernel_not_found:	db 'KERNEL.BIN file not found error', ENDL, 0
